...
public class SimpleToolCallingWithStreamingExample {
    ...
    public static void askModel(String ollamaHost, String modelName) throws ToolInvocationException, OllamaBaseException, IOException, InterruptedException, URISyntaxException {
        ...
        String prompt = new Tools.PromptBuilder()
                ...
                .build();
        OllamaStreamHandler handler = new OllamaStreamHandler() {
            @Override
            public void accept(String message) {
                System.out.print(message.toUpperCase());
            }
        };
        OllamaToolsResult toolsResult = ollamaAPI.generateWithTools(modelName, prompt, new OptionsBuilder().build(), handler);
